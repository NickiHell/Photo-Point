...
try:
    ...
    async def get_delivery(
        delivery_id: str, delivery_repository=Depends(get_delivery_repository)
    ):
        """Get delivery by ID."""
        try:
    ...
    @router.get(
        "/notification/{notification_id}", response_model=list[DeliveryResponse]
    )
    async def get_deliveries_by_notification(
        notification_id: str, delivery_repository=Depends(get_delivery_repository)
    ):
    ...
    async def get_pending_retries(delivery_repository=Depends(get_delivery_repository)):
    ...
    router: Any = MockRouter()  # type: ignore
