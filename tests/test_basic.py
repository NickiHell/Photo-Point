...
from src.service import NotificationService, DeliveryStrategy
...
class TestNotificationService(unittest.TestCase):
    ...
    def test_first_success_strategy(self):
        ...
        async def run_test():
            ...
            self.assertEqual(report.total_attempts, 2)  # Должен остановиться после успеха
            self.assertEqual(providers[0].call_count, 1)
            self.assertEqual(providers[1].call_count, 1)
    ...
    def test_try_all_strategy(self):
        ...
        async def run_test():
            ...
            self.assertTrue(report.success)
            self.assertEqual(report.total_attempts, 3)  # Должен попробовать все
